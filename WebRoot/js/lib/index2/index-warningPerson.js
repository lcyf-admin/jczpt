	...
	function loadContrastPerson(stationId){
		...
		$.ajax({
			type: "POST",
			url: "contrastPerson_ajaxContrastPersonList.action",
			...
			success: function(data){
				...
				for(var i=0; i<data.length; i++){
					...
					$("#warnPersonList_"+i+"_1").text(data[i].idcardOmit);
					$("#warnPersonList_"+i+"_2").text(data[i].captureTimeView);
					$("#warnPersonList_"+i+"_3").text(data[i].actionOmit);
					$("#warnPersonList_"+i+"_4").text(data[i].locationOmit);
					$("#warnPersonList_"+i+"_5").text(data[i].isCheckedView);
				}
			},
	...
	function addContrastPerson(data){
		for(var i=3;i>0;i--){
			...
			$("#warnPersonList_"+i+"_5").text($("#warnPersonList_"+(i-1)+"_5").text());
		}
		$("#warnPersonList_"+i+"_0").empty().append("<a class='lk-index-link' href=\"javascript: dealWarnPerson('"+data.id+"')\";>"+data.namesOmit+"</a>");
		$("#warnPersonList_"+i+"_1").text(data.idcardOmit);
		$("#warnPersonList_"+i+"_2").text(data.captureTimeView);
		$("#warnPersonList_"+i+"_3").text(data.actionOmit);
		$("#warnPersonList_"+i+"_4").text(data.locationOmit);
		$("#warnPersonList_"+i+"_5").text(data.isCheckedView);
