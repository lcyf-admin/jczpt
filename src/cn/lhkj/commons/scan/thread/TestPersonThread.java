 ...
 * 线程监听车证合一采集的车辆是否允许通过
 */
public class TestPersonThread implements Runnable {
	
	private static final Logger logger = Logger.getLogger(TestPersonThread.class);
	...
	public void run(){
		try {
			...
			while(true){
				...
				person.setLocation(randomDoorName());
				person.setAddress("XXX省XX市XX县XXX小区XX号楼XX单元");
				person.setCaptureTime(new Date());
				person.setStartDate("2010-12-3");
				person.setEndDate("2030-12-3");
				person.setEquipmentId(randomDoorId());
				person.setIdcard(randomIdcard());
				person.setGender(IdCardGenerator.gender(person.getIdcard()));
				person.setLocalImgUrl(randomPersonImage());
				person.setIsAPerson("1");
				person.setNames(randomName());
				dao.save(person);
	...
	/**随机人员照片*/
	private String randomPersonImage(){
		Random random = new Random();
